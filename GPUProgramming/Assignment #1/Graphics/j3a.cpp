...
bool loadJ3A( const char* fn ) {
	...
	for( auto ptr: obj_vertices ) delete ptr; obj_vertices.clear();
	for( auto ptr: obj_normals ) delete ptr; obj_normals.clear();
	...
	obj_vertices.resize( nObjects );
	obj_normals.resize( nObjects );
	...
	for( int i=0; i<nObjects; i++ ) {
		...
		obj_vertices[i]		= new vec3[nVertices[i]];
		obj_normals[i]		= new vec3[nVertices[i]];
		texCoords[i]	= new vec2[nVertices[i]];
		
		for( unsigned j=0; j<nVertices[i]; j++ )
			ifs>>obj_vertices[i][j].x>>obj_vertices[i][j].y>>obj_vertices[i][j].z;
		for( unsigned j=0; j<nVertices[i]; j++ )
			ifs>>obj_normals[i][j].x>>obj_normals[i][j].y>>obj_normals[i][j].z;
		for( unsigned j=0; j<nVertices[i]; j++ )
